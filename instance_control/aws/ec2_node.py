...
_LOG = logging.getLogger('bubuku.cluster.create_instance')
...
def _launch_instance(region: str, ip: dict, ami: object, subnet: dict, security_group_id: str, cluster_config: dict):
    _LOG.info('Launching node %s', ip['_defaultIp'])
    ...
    if cluster_config['create_ebs']:
        _create_tagged_volume(ec2_client, cluster_config, subnet['AvailabilityZone'], KAFKA_LOGS_EBS)
...
def _launch_nodes(cluster_config: dict):
    ...
    for ip in cluster_config['node_ips']:
        ...
        if i + 1 == cluster_config['cluster_size']:
            _LOG.info("Done. Node(s) is created.")
            break
        i += 1
...
def create(cluster_config: dict):
    ...
    try:
        ...
        cluster_config['instance_profile'] = iam.create_instance_profile(cluster_config)
